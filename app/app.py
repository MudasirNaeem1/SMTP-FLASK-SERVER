...
def get_db_connection():
    conn = sqlite3.connect(DB_PATH, check_same_thread=False)  # Allowing multiple threads to access the DB
    return conn
...
def home():
    ...
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT id, sender, subject, body FROM inbox WHERE recipient=? AND is_important=1", (session['email'],))
...
def compose():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            conn = get_db_connection()
            c = conn.cursor()
            c.execute("INSERT INTO sent (sender, receiver, subject, body) VALUES (?, ?, ?, ?)",
...
def inbox():
    ...
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT id, sender, subject, body FROM inbox WHERE recipient=?", (session['email'],))
    mails = c.fetchall()
    conn.close()
...
def sent():
    ...
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT id, receiver, subject, body FROM sent WHERE sender=?", (session['email'],))
    mails = c.fetchall()
    conn.close()
